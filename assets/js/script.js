var DOMLoaded = function () {
  ...
  function getMarkupFromData(dataForSingleItem) {
    ...
    var criticNamesNew = Object.fromEntries(Object.entries(floatCriticNames).sort(([, a], [, b]) => b - a));
    ...
    dateFormattedFilter = statusFilter(isDateIncludedlast7Days, isDateIncludedlast14Days, isDateIncludedlast30Days, isDateIncludedlast6Months, isDateIncluded2022, isDateIncluded2019, isDateIncluded2020, isDateIncluded2021);
    ...
    if (retrieveLocalData(criticActive)) {
      ...
      if (critic == 0) {
        ...
        criticDetails =
          parseFloat(critic)
            .toFixed(2)
            .replace(/([0-9]+(\.[0-9]+[1-9])?)(\.?0+$)/, "$1") + "<span>/5</span>";
      }
    ...
    var ratingTwitter = rating.replace("<span>/5</span>", "").replace(".", ",");
    ...
    return [
      '<figure id="' +
        urlId +
        '" class="col-3@xs col-4@sm col-3@md picture-item shuffle-item shuffle-item--visible" data-genre="' +
        genre +
        '" data-nationality="' +
        nationality +
        '" data-duration="' +
        ...
        '" data-seasons-critic="' +
        seasonsCriticArray +
        '" data-seasons-critic-details="' +
        seasonsCriticDetailsArray +
        '" data-critic-keys="' +
        criticNamesKeys +
        '" data-critic-values="' +
        criticNamesValues +
  ...
  function splitDate(date) {
    ...
    if (newDate[0] == "Mon" || newDate[0] == "Tue" || newDate[0] == "Wed" || newDate[0] == "Thu" || newDate[0] == "Fri" || newDate[0] == "Sat" || newDate[0] == "Sun") {
      altFormat = false;
    }
  ...
  function statusFilter(isDateIncludedlast7Days, isDateIncludedlast14Days, isDateIncludedlast30Days, isDateIncludedlast6Months, isDateIncluded2022, isDateIncluded2019, isDateIncluded2020, isDateIncluded2021) {
    var text = "",
      text2 = "";
  ...
  function twitterTops(title1, rating1, title2, rating2, title3, rating3) {
    ...
    text += "%0a%0ahttps://yaquoiaucine.github.io";
    text += "%0a%0a%23YQAC";
  ...
  function hashtagFormatted(string) {
    return string.replace(/\s+/g, "").replace("'", "").replace("?", "").replace(",", "").replace("-", "").replace(":", "");
  }
  ...
  function displayOverlay() {
    ...
    imgsLink.forEach(function (link) {
      link.addEventListener(
        "click",
        function (e) {
          if (e.target.classList.contains("movieSeen") || e.target.classList.contains("movieSeenIcon") || e.target.classList.contains("movieSeenIconSmall")) {
            return;
          }
          ...
          if (criticKeysNew.includes("Note AlloCiné") || criticKeysNew.includes("Note BetaSeries") || criticKeysNew.includes("Note IMDb")) {
          ...
          if (criticKeysNew.includes("Note BetaSeries") && parseFloat(criticValuesNew[criticKeysNew.indexOf("Note BetaSeries")]) !== 0) {
          ...
          if (criticKeysNew != "" && criticKeysNew[0] != "Note AlloCiné" && criticKeysNew[0] != "Nombre notes AlloCiné" && criticKeysNew[0] != "Note BetaSeries" && criticKeysNew[0] != "Note IMDb" && criticKeysNew[0] != "Nombre notes IMDb") {
            ...
            for (var htmlTagCriticRecapIndex = 0; htmlTagCriticRecapIndex < criticKeysNewLength; htmlTagCriticRecapIndex++) {
              if (criticKeysNew[htmlTagCriticRecapIndex] != "Note AlloCiné" && criticKeysNew[htmlTagCriticRecapIndex] != "Nombre notes AlloCiné" && criticKeysNew[htmlTagCriticRecapIndex] != "Note BetaSeries" && criticKeysNew[htmlTagCriticRecapIndex] != "Note IMDb" && criticKeysNew[htmlTagCriticRecapIndex] != "Nombre notes IMDb") {
    ...
    overlay.addEventListener("click", function (e) {
      if (e.target.localName == "div" || e.target.id == "close-button") {
  ...
  function defaultInputClick() {
    ...
    if (activeSort == "note") {
      ...
      localStorage.setItem("yqac_sort." + "defaultInput", "popularity");
    } else if (activeSort == "date_de_creation" || activeSort == "date_creation" || activeSort == "creation") {
      localStorage.setItem("yqac_sort." + "defaultInput", "creationdate");
    } else {
  ...
  function toggleLocalData(item) {
    ...
    if (classListName == "criticAllocine" || classListName == "usersAllocine" || classListName == "usersImdb" || classListName == "usersBetaseries") {
      localStorage.setItem("yqac_menu." + "menuBool", true);
    }
  ...
  function menuButtonsChecked() {
    ...
    menuAllCheckedArray.forEach(function (button) {
      ...
      button.children[0].children[0].addEventListener(
        "click",
        function () {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            Swal.fire({
              ...
              localStorage.clear();
              window.location = "https://yaquoiaucine.github.io";
            });
          }
        });
      },
  ...
  function setMenuButtonAll() {
    ...
    criticToggle0.addEventListener(
      "click",
      function () {
        localStorage.setItem("yqac_menu." + "menuBool", true);
    ...
    nationalityButton0.addEventListener(
      "click",
      function () {
        nationalityArray.forEach(function (nationality) {
    ...
    durationButton0.addEventListener(
      "click",
      function () {
        durationArray.forEach(function (duration) {
  ...
  var theaterSearchOnChange = function (evt) {
    ...
    theatersRes(evt).then(function (autoCompleteResult) {
      document.getElementById("result").innerHTML = autoCompleteResult.map((i) => `<a href="https://yaquoiaucine.github.io?cinemaId=${i.theaterNameCode}" class="theaterResLink">${i.theaterName}</a>`).join("");
