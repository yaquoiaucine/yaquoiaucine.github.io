...
abord_script () {
  ...
  curl -s https://yaquoiaucine.surge.sh/assets/js/data.json > ./assets/js/data.json
...
curl -s $baseUrl > temp
curl -s https://yaquoiaucine.surge.sh/assets/sh/checkingFile.txt > ./assets/sh/checkingFile.txt
